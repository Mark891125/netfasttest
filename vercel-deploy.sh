...
check_project_config() {
    ...
    if [[ -f "vercel.json" ]]; then
        echo -e "${YELLOW}检查 vercel.json 语法...${NC}"
        if command -v jq &> /dev/null; then
            if jq empty vercel.json 2>/dev/null; then
                echo -e "${GREEN}✅ vercel.json 语法正确${NC}"
                ...
                if jq -e '.builds and .functions' vercel.json >/dev/null 2>&1; then
                    ...
                    echo -e "${YELLOW}💡 移除 builds 属性，Vercel会自动检测Next.js项目${NC}"
                    exit 1
                fi
            else
            ...
            echo -e "${YELLOW}⚠️  jq 未安装，跳过 vercel.json 语法检查${NC}"
        fi
    else
